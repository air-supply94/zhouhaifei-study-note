...
- 生产者和消费者线程共享一个 n 个槽的缓冲区.生产者反复生成新的条目,消费者不断取出条目
- 插入和读取都是互斥
- 如果缓冲区是满的,生产者必须等待

### 读者和写者

- 一组并发线程访问共享变量,有些线程只读对象,有些线程只写对象
- 读者优先: 要求不让读者等待,除非已经把使用对象的权限赋予一个写者
- 写者优先: 要求一旦一个写者准备可以写,就尽可能完成写的操作
- 饥饿: 一个线程无限期的阻塞,无法进展

### 线程不安全函数类

- 不保护共享变量的函数
- 保持跨越多个调用状态的函数(伪随机生成器)
- 返回指向静态变量的指针函数
- 调用线程不安全的函数

### 竞争

- 当一个程序的正确行依赖于一个线程要在另一个线程到达 y 点之前到达它的控制流的 x 点
- 对等线程的程序必须对任何可行的轨迹线都正确工作

### 死锁

一个线程被阻塞,等待一个永远不为真的条件

- P 和 V 操作顺序不当,以至于两个信号量的禁止区域重合
- 死锁区域:2 个禁止区有重叠区域的左下方和 2 个禁止区开始边界
- 加锁顺序: 给定所有互斥操作的一个全序,如果每个线程都是以一种顺序获得互斥锁并以相反的顺序释放
